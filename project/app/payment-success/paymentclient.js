    ...
    useEffect(() => {
        ...
        const checkPaymentStatus = async () => {
            try {
                ...
                const subscriptionResponse = await axios.get(`/api/paymentdetails/${paymentId}`);
                ...
                if (subscriptionResponse && subscriptionResponse.data) {
                    ...
                    try {
                        const mentorResponse = await axios.get(`/api/user/${subscriptionData.mentor}`);
                        setMentorDetails(mentorResponse.data);
                    } catch (mentorError) {
                        console.error('Error fetching mentor details:', mentorError);
                        // Continue even if mentor details fail
                    }
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div className="w-full bg-gray-50 p-4 rounded-lg mb-6">
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="text-sm font-medium text-right capitalize">{subscriptionDetails.plan}</div>

                                <div className="text-sm text-gray-500">Amount:</div>
                                <div className="text-sm font-medium text-right">{formatCurrency(subscriptionDetails.price)}</div>

                                <div className="text-sm text-gray-500">Start Date:</div>
                                <div className="text-sm font-medium text-right">{formatDate(subscriptionDetails.startDate)}</div>

                                <div className="text-sm text-gray-500">End Date:</div>
                                <div className="text-sm font-medium text-right">{formatDate(subscriptionDetails.endDate)}</div>

                                {mentorDetails && (
                                    <>
                                        <div className="text-sm text-gray-500">Mentor:</div>
                                        <div className="text-sm font-medium text-right">{mentorDetails.name}</div>
                                    </>
                                )}

                                <div className="text-sm text-gray-500">Payment ID:</div>
                                <div className="text-sm font-medium text-right">{subscriptionDetails.paymentId}</div>
                            </div>
                        </div>

                        <p className="text-gray-600 text-center mb-6">
                            Thank you for your subscription. You now have access to mentor sessions until {formatDate(subscriptionDetails.endDate)}.
                        </p>
