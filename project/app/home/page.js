...
const mentors = [
  {
    name: "Ayla S.",
    role: "Top-rated marketing expert",
    price: "$150/month",
    services: [
...
const Home = () => {
  return (
    <div className="font-inter text-gray-900 bg-white">
      {/* Navbar */}
      <nav className="fixed top-0 left-0 w-full bg-white shadow-md py-4 px-4 md:px-12 flex flex-row justify-between items-center z-50">
        <h1 className="text-2xl md:text-3xl font-bold text-gray-900">MentorClone</h1>
        <div className="flex space-x-4 md:space-x-8">
          <button className="text-gray-800 font-medium hover:text-blue-600">Log in</button>
          <button className="bg-blue-600 text-white px-4 md:px-6 py-2 rounded-lg text-base md:text-lg font-semibold hover:bg-blue-700 transition">Sign Up</button>
        </div>
      </nav>

      {/* Category Navigation */}
      <div className="bg-gray-100 py-4 mt-16 shadow-sm overflow-x-auto">
        <div className="container mx-auto flex justify-start md:justify-center px-4 md:px-0 space-x-6 md:space-x-8 text-gray-700 font-medium whitespace-nowrap">
      ...
      {/* Hero Section */}
      <header className="bg-blue-600 text-white text-center pt-20 md:pt-40 pb-16 md:pb-28 px-4 md:px-6">
        <motion.h1
          className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold leading-tight max-w-4xl mx-auto"
        ...
        <motion.p
          className="text-lg md:text-xl mt-4 max-w-2xl mx-auto opacity-80"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        <div className="mt-8 flex justify-center">
          <div className="w-full max-w-xl px-4 flex">
            <input
              ...
              className="w-full p-3 md:p-4 rounded-l-lg border border-blue-300 text-gray-800 focus:outline-none bg-white shadow-sm"
            />
            <button className="bg-yellow-500 text-white px-4 md:px-6 py-3 md:py-4 rounded-r-lg font-semibold hover:bg-yellow-600 transition whitespace-nowrap">Search</button>
          </div>
        </div>
      </header>
      ...
      <section className="py-12 md:py-16 bg-white">
        <div className="container mx-auto text-center px-4 sm:px-6 lg:px-12">
          ...
          {/* Section Title */}
          <h2 className="text-2xl sm:text-3xl font-extrabold text-[#142245] mb-8 md:mb-10">
      ...
      <section className="bg-[#142245] py-12 md:py-16 px-4 sm:px-8 lg:px-16 text-white">
        <div className="container mx-auto text-center max-w-6xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mt-8 md:mt-10">
            {[
              ...
              <div key={index} className="bg-white text-black p-5 md:p-6 rounded-lg shadow-md transition-all hover:shadow-lg h-full flex flex-col">
                <h3 className="text-lg sm:text-xl font-semibold">{session.title}</h3>
                <p className="text-gray-600 mt-2 text-sm sm:text-base flex-grow">{session.desc}</p>
                <div className="mt-4">
                  <p className="font-semibold text-green-600">{session.duration}</p>
        ...
        <div className="container mx-auto mt-16 md:mt-20 max-w-6xl">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {testimonials.slice(0, 6).map((testimonial, index) => (
                <div key={index} className="bg-white text-black p-5 md:p-6 rounded-lg shadow-md transition-all hover:shadow-lg h-full">
      ...
      <section className="container mx-auto py-12 md:py-20 px-4 md:px-6">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-8 md:mb-12">Featured Mentors</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-12 text-center">
          {featuredMentors.map((mentor, index) => (
            ...
            >
              ...
              />
              <h3 className="text-xl md:text-2xl font-semibold">{mentor.name}</h3>
              <p className="text-gray-600 text-base md:text-lg">{mentor.role}</p>
